		...
		if (w_row_total > wt_min || lastRow) {
			...
			for (j = 0; j < w_row.length; j++) {
				...
				if (img_url.match(/.bg-\d+/)) {
					new_img_url = img_url.replace(/.bg-\d+/, '.bg-'+new_width);
					resized = true;
				}
				else if (img_url.match(/w=\d+/)) {
					new_img_url = img_url.replace(/w=\d+/, 'w='+new_width).replace(/h=\d+/, 'h='+Math.round(h_row[j]*new_width/w_row[j]));
					resized = true;
				}
				...
				if (resized) {
					...
					var new_style = old_style + "; background-image: url('" + new_img_url + "'), url('/ApacheGallery/modern/squares.svg');";
					elem.attr('style', new_style);

					// Event to remove SVG spinner once image is loaded, as it uses a lot of CPU even when obscured.
					$('<img/>').attr('src', new_img_url).data('target', elem).load(function() {
